...
<body class="min-h-screen bg-slate-50 text-slate-950 antialiased">
    <header class="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 backdrop-blur">
        <nav class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-3 sm:px-8 lg:px-10" aria-label="Navigasi utama">
            ...
            <a href="/admin" class="rounded-lg border border-teal-200 px-3 py-2 text-sm font-semibold text-teal-800 transition hover:border-teal-700 hover:bg-teal-50" aria-label="Masuk ke panel admin">
                Admin
            </a>
        </nav>
    </header>
    ...
    <main id="beranda">
                        ...
                        <div class="mt-5 grid grid-cols-2 gap-3">
                            @foreach ($applications->take(4) as $application)
                                <div class="rounded-lg bg-white p-4 shadow-sm ring-1 ring-slate-100">
                                    <span class="flex h-10 w-10 items-center justify-center rounded-lg bg-teal-700 text-sm font-bold text-white">{{ str($application->short_name ?: $application->name)->substr(0, 2)->upper() }}</span>
        ...
        @if ($announcements->isNotEmpty())
            <section class="mx-auto max-w-7xl px-5 pt-6 sm:px-8 lg:px-10" aria-label="Pengumuman">
                <div class="space-y-3">
